// Comprehensive water product database compilation
...
function getSourceType(name) {
  ...
  if (lower.includes('alkaline')) return 'municipal'; // Often processed
...
function getBrandFromName(name) {
  ...
  return words[0]; // First word is usually the brand
}

// Collected data from all tabs
const allProducts = [
  // All bottled waters (20 products)
  ...
  // Sparkling waters (20 products - some overlap with above)
...
allProducts.forEach(product => {
  ...
  if (!seen.has(key)) {
    ...
    const processed = {
      id: null, // Will be filled when we get detailed data
      name: product.name,
      score: product.score,
      type: product.type,
      image: null,
      ...
      details: null // Will be filled with detailed data later
    };
...
console.log(`Total unique products collected: ${processedProducts.length}`);
console.log(`By type:`);
const typeCount = {};
